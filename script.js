...
async function init(){
  ...
  }catch(e){ loader.textContent='❌ cartones'; return; }
  pintar(); observar();

  /* 2 – reservas (JSONP, para saltar CSP) */
...
form.addEventListener('submit',e=>{ e.preventDefault(); const fd=new FormData(form); const id=fd.get('ID'); if(bloqueado(id)){alert('Cartón bloqueado o reservado');return;} reservar(fd,id);});
function reservar(fd,id){ const ifr=document.createElement('iframe'); ifr.name='hidden_iframe'; ifr.style.display='none'; document.body.appendChild(ifr); const f=document.createElement('form'); f.action=WEBAPP_URL; f.method='POST'; f.target='hidden_iframe'; fd.forEach((v,k)=>{const inp=document.createElement('input'); inp.name=k; inp.value=v; f.appendChild(inp);}); document.body.appendChild(f); f.submit(); vendidos.add(id); refrescar(); window.open(`https://wa.me/${WHATS_APP}?text=${encodeURIComponent('Hola, quiero comprar el cartón '+id)}`,'_blank'); btnDesc.classList.remove('hidden'); btnDesc.onclick=()=>jpg(id); cerrar(); }
